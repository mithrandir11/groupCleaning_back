...
<div class="grow">
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش اعلان</h1>
    ...
    <form action="{{route('admin.notifications.update', $notification)}}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')

        <div>
            <label for="title" class="block mb-2 text-sm font-medium text-gray-900">عنوان</label>
            <input name="title"  value="{{ old('title', $notification->title) }}" type="text" class="max-w-sm text-gray-500 text-sm border border-gray-300  rounded-lg outline-none focus:border-blue-500 block w-full p-3">
            @error('title') 
        ...
        <div>
            ...
            <textarea name="message" rows="6" class="max-w-sm text-gray-500 text-sm border border-gray-300  rounded-lg outline-none focus:border-blue-500 block w-full p-3">{{ old('message', $notification->message) }}</textarea>
            @error('message') 
                <span class="text-red-500 text-sm">{{$message}}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="role_id" class="max-w-sm border border-gray-300 text-gray-900 text-sm rounded-lg outline-none focus:border-blue-500 block w-full p-2.5 ">
                ...
                <option value="{{$role->id}}" @selected($role->id == $notification->role_id)>{{ __('fa.role.'.$role->name) }}</option>   
                @endforeach
              </select>
            @error('role') 
                <span class="text-red-500 text-sm">{{$message}}</span>
