...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش خدمت</h1>
    ...
    <form action="{{route('admin.services.update', $service)}}" method="POST" class="space-y-12" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-right mb-1" for="title">عنوان انگلیسی</label>
        ...
        <div>
            <label class="block text-right mb-1" for="title_fa">عنوان فارسی</label>
            <input type="text" name="title_fa" id="title_fa" 
                   class="border rounded-lg w-full p-3 max-w-sm"
                   value="{{ old('title_fa', $service->title_fa) }}" >
            @error('title_fa')
                <p class="text-red-600 text-sm ">{{ $message }}</p>
            @enderror
        </div>
