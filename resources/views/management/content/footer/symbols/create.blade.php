...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ایجاد نماد جدید</h1>
    ...
    <form action="{{route('admin.footerSymbol.store')}}" method="POST" class="space-y-8">
        @csrf
        <div>
        ...
        <div >
            <textarea name="text" rows="6" class="border rounded-lg w-full p-3 max-w-sm">{{ old('text') }}</textarea>
            @error('text')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
