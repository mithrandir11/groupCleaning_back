...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش نماد جدید</h1>
        <a href="{{route('admin.footer')}}" class="flex items-center gap-x-2 border duration-200 rounded-lg text-sm px-4 py-2 ">
            بارگشت
    ...
    <form action="{{route('admin.footerSymbol.update', $symbol)}}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        <div>
        ...
        <div >
            <textarea name="text" rows="6" class="border rounded-lg w-full p-3 max-w-sm">{{ old('text', $symbol->text) }}</textarea>
            @error('text')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
