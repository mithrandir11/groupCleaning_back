...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش منو</h1>
        <a href="{{route('admin.menu')}}" class="flex items-center gap-x-2 border duration-200 rounded-lg text-sm px-4 py-2 ">
            بارگشت
    ...
    <form action="{{ route('admin.menu.update', $menu->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div >
            <textarea id="myeditorinstance" name="text">{{ old('text', $menu->text) }}</textarea>
            @error('text')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="seo[title]" id="seo_title" class="border rounded-lg w-full p-3 max-w-sm" value="{{ $menu->seo?->title }}" >
            @error('seo.title')
                <p class="text-red-600 text-sm ">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea  name="seo[description]" id="seo_description" class="border rounded-lg w-full p-3 max-w-sm">{{  $menu->seo?->description }}</textarea>
        ...
        <div>
            ...
            <input type="text" name="seo[keywords]" id="seo_keywords" class="border rounded-lg w-full p-3 max-w-sm" value="{{ old('seo.keywords', $menu->seo?->keywords) }}" >
            {{-- <p class="text-red-600 text-sm ">{{ $message }}</p> --}}
            @error('seo.keywords')
                <p class="text-red-600 text-sm ">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" name="seo[canonical_url]" id="canonical_url" class="border rounded-lg w-full p-3 max-w-sm" value="{{ old('seo.canonical_url', $menu->seo?->canonical_url) }}" >
