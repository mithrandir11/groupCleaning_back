...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش سوال </h1>
        <a href="{{route('admin.faqs.show', $faq->menu_id)}}" class="flex items-center gap-x-2 border duration-200 rounded-lg text-sm px-4 py-2 ">
            بارگشت
    ...
    <form action="{{route('admin.faqs.update', $faq)}}" method="POST" class="space-y-8">
        ...
        <div>
            <label class="block text-right mb-1" for="question">سوال</label>
        ...
        <div >
            ...
            <select name="menu_id" id="menu_id" class="border rounded-lg  w-full p-3 max-w-sm">
                ...
                @foreach($allMenus as $m)
                    <option value="{{ $m->id }}" {{ $faq->menu_id == $m->id ? 'selected' : '' }}>
                        {{ $m->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div >
            <label class="block text-right mb-1" for="answer">جواب</label>
            <textarea id="myeditorinstance" name="answer">{{old('answer', $faq->answer)}}</textarea>
            @error('answer')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
