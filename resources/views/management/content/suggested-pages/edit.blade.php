...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش صفحه پیشنهادی</h1>
        <a href="{{route('admin.suggestedPages.show', $suggested_page->menu_id)}}" class="flex items-center gap-x-2 border duration-200 rounded-lg text-sm px-4 py-2 ">
            بارگشت
    ...
    <form action="{{route('admin.suggestedPages.update', $suggested_page)}}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        <div>
            ...
            <input type="text" name="title" id="name" 
                   ...
                   value="{{ old('title', $suggested_page->title) }}" >
            @error('title')
        ...
        <div >
            ...
            <input type="text" name="url" id="url" 
                   class="border rounded-lg  w-full p-3 max-w-sm"
                   value="{{ old('url', $suggested_page->url) }}" >
            @error('url')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div >
            ...
            <select name="menu_id" id="menu_id" class="border rounded-lg  w-full p-3 max-w-sm">
                ...
                @foreach($allMenus as $m)
                    <option value="{{ $m->id }}" {{ $suggested_page->menu_id == $m->id ? 'selected' : '' }}>
                        {{ $m->name }}
                    </option>
                @endforeach
            </select>
