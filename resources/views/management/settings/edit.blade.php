...
<div class="container mx-auto px-4" >
    ...
    <div class="flex justify-between items-center gap-x-3 mb-10 ">
        <h1 class="text-2xl font-bold mb-4 text-right">ویرایش تنظیمات </h1>
        <a href="{{route('admin.settings')}}" class="flex items-center gap-x-2 border duration-200 rounded-lg text-sm px-4 py-2 ">
            بارگشت
    ...
    <form action="{{route('admin.settings.update', $setting)}}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-right mb-1" for="key">کلید</label>
            <input disabled type="text" name="key" id="key" 
                   class="disabled border rounded-lg w-full p-3 max-w-sm"
        ...
        <div>
            <label class="block text-right mb-1" for="value">مقدار</label>
        ...
        <div>
            <label class="block text-right mb-1" for="description">توضیحات</label>
            <textarea rows="5" name="description" class="border rounded-lg w-full p-3 max-w-sm">{{ old('description', $setting->description) }}</textarea>
            @error('description')
                <p class="text-red-600 text-sm ">{{ $message }}</p>
            @enderror
        </div>
