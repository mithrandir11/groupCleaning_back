...
Route::prefix('admin')->group(function () {
    ...
    Route::get('/verify-otp', [AdminAuthController::class, 'showVerifyForm'])->name('admin.verify.otp');
    Route::post('/verify-otp', [AdminAuthController::class, 'verifyOtp']);
    Route::post('/resend-otp', [AdminAuthController::class, 'resendOtp'])->name('admin.resend.otp')->middleware('throttle:3,1');//3 request per 1 minutes
});

// ورود نیروی کار
Route::prefix('worker')->group(function () {
    Route::get('/login', [WorkerAuthController::class, 'showLoginForm'])->name('worker.login.form');
    Route::post('/send-otp', [WorkerAuthController::class, 'sendOtp'])->name('worker.send.otp');
    Route::get('/verify-otp', [WorkerAuthController::class, 'showVerifyForm'])->name('worker.verify.otp');
    Route::post('/verify-otp', [WorkerAuthController::class, 'verifyOtp']);
    Route::post('/resend-otp', [WorkerAuthController::class, 'resendOtp'])->name('worker.resend.otp')->middleware('throttle:3,1');//3 request per 1 minutes
});
